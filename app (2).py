...
# حد أدنى للتشابه الدلالي الخام (قبل التطبيع) لاعتبار السياق ذا صلة كافية بالسؤال.
# لو أعلى تشابه دلالي بين السؤال وأي مستند أقل من هذا الرقم، يُعتبر السؤال خارج نطاق
# قاعدة المعرفة، ولا يتم استدعاء نموذج التوليد أصلاً.
RELEVANCE_THRESHOLD = 0.35

NO_ANSWER_MESSAGE = (
    "🤔 معنديش معلومات كافية عن ده في قاعدة المعرفة الحالية عن أسوان. "
    "جرب تسأل عن المعابد، الحدائق، النزهات النيلية، أو عادات وتقاليد النوبة."
)
...
def retrieve(query, method, k=3, alpha=0.6):
    ...
    # التشابه الدلالي الخام (غير مطبَّع) هو مقياس مطلق موثوق لمدى صلة السؤال بقاعدة
    # المعرفة، بعكس أي score مطبَّع بين 0 و1 واللي بيدي أعلى مستند قيمة قريبة من 1
    # دايماً حتى لو مفيش صلة حقيقية. بنستخدمه هنا كبوابة قبل استدعاء نموذج التوليد.
    max_semantic = float(semantic_scores(query).max())
    return context, max_semantic


SYSTEM_INSTRUCTIONS = """أنت مرشد سياحي وثقافي متخصص في مدينة أسوان فقط، تتحدث بأسلوب ودود وواضح بالعربية.

قواعد صارمة يجب اتباعها دائماً:
1. أجب فقط بالاعتماد على المعلومات الموجودة حرفياً في "السياق المسترجع" الممرَّر إليك في كل رسالة.
2. ممنوع استخدام أي معلومة من معرفتك العامة عن أسوان أو مصر أو أي موضوع آخر، حتى لو كنت تعرفها وتبدو صحيحة.
3. إذا كان السياق المسترجع لا يحتوي على إجابة كافية أو غير مرتبط بسؤال المستخدم، يجب أن يكون ردك حرفياً:
"معنديش معلومات كافية عن ده في قاعدة المعرفة الحالية عن أسوان."
ولا تضف أي تخمين أو معلومة إضافية بعدها.
4. لا تعتذر بشكل مبالغ فيه ولا تشرح لماذا لا تملك المعلومة — فقط صرّح بذلك بإيجاز."""
...
السياق المسترجع من قاعدة المعرفة (المصدر الوحيد المسموح به للإجابة):
{context_text}
...
if query:
    ...
    with st.chat_message("assistant", avatar="🏛️"):
        with st.spinner("جاري الاسترجاع..."):
            context, max_semantic = retrieve(
                query, active_mode["retriever"], k=active_mode["k"], alpha=active_mode["alpha"]
            )

        is_relevant = max_semantic >= RELEVANCE_THRESHOLD

        if not is_relevant:
            # السؤال بعيد عن قاعدة المعرفة — نرفض الإجابة فوراً من غير ما ننادي أي موديل توليدي،
            # عشان منديش فرصة لأي نموذج (محلي أو سحابي) يحاول يجاوب من معرفته العامة.
